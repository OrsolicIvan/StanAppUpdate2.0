...
namespace APARTMENTS.Controller
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<ActionResult<User>> AddRentedApartment(RentApDto rentedApartment)
        {
            var user = await _context.Users
                .Include(x => x.OwnedApartments)
                .SingleOrDefaultAsync(x => x.Id == rentedApartment.RenterId);
            var apartment = await _context.Apartments.Where(i => i.Id == rentedApartment.ApartmentId).SingleOrDefaultAsync();
            if (user == null)
                return NotFound();
            if (apartment != null)
            {
